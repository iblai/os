...
export function SubmitMessageButton({
  ...
  const isDisabled =
    disabled || (isPreviewMode && !allowAnonymousAccess) || isUploading || isConnecting;
      ...
      <TooltipTrigger asChild>
        <div>
          ...
          >
            ...
            <span className="sr-only">{isConnecting ? 'Connecting' : 'Send message'}</span>
          </Button>
        </div>
      </TooltipTrigger>
      <TooltipContent className="ibl-tooltip-content capitalize">{getTooltipText()}</TooltipContent>
    </Tooltip>
  );
}

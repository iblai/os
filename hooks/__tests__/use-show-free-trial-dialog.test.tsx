...
function Harness({ options, onReady }: { options?: any; onReady: (value: ReturnType<typeof useShowFreeTrialDialog>) => void }) {
...
beforeEach(() => {
  mockReduxState = {
    subscription: { subscriptionStatus: { creditExhausted: false, callToAction: undefined } },
...
describe("useShowFreeTrialDialog", () => {
  ...
  it("prefers custom modal component over fallback", () => {
    ...
    render(<Harness options={{ modalComponent: CustomModal }} onReady={(api) => (hookApi = api)} />);

    expect(hookApi!.FreeTrialDialog).toBe(CustomModal);
  });

  it("returns fallback dialog when enabled, otherwise null", () => {
    let apiWithFallback: ReturnType<typeof useShowFreeTrialDialog> | null = null;
    render(<Harness onReady={(api) => (apiWithFallback = api)} />);
    expect(apiWithFallback!.FreeTrialDialog).not.toBeNull();

    let apiWithoutFallback: ReturnType<typeof useShowFreeTrialDialog> | null = null;
    render(<Harness options={{ enableFallbackModal: false }} onReady={(api) => (apiWithoutFallback = api)} />);

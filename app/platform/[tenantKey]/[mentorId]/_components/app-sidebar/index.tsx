  ...
  const handleSelectMessage = async (message: any) => {
    ...
    const messages = message.messages
      .map((messageObj) => {
        ...
        const additionalKwargs = messageObj.message?.data?.additional_kwargs;
        const reasoningSummaries = additionalKwargs?.reasoning?.summary;
        ...
        const toolCalls = Array.isArray(toolOutputs)
          ? toolOutputs.map((tool: any) => ({
              id: tool.id || "",
              name: tool.type || "",
              log: tool.action?.query || "",
              result: tool.status || "",
            }))
          : undefined;

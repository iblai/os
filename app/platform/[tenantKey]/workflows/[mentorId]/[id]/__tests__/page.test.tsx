...
vi.mock('@iblai/iblai-js/data-layer', () => ({
  ...
  useDeactivateWorkflowMutation: () => [mockDeactivateWorkflow, { isLoading: false }],
  useActivateWorkflowMutation: () => [mockActivateWorkflow, { isLoading: false }],
  useDeleteWorkflowMutation: () => [mockDeleteWorkflow, { isLoading: false }],
  useValidateWorkflowMutation: () => [mockValidateWorkflow, { isLoading: false }],
...
vi.mock('@/components/ui/input', () => ({
  Input: ({ onChange, value, onBlur, onKeyDown, ...props }: React.ComponentProps<'input'>) => (
    <input onChange={onChange} value={value} onBlur={onBlur} onKeyDown={onKeyDown} {...props} />
  ),
}));
...
vi.mock('@/components/workflows/workflow-preview-chat', () => ({
  WorkflowPreviewChat: ({ tenantKey, mentorId }: { tenantKey: string; mentorId?: string }) => (
    <div data-testid="workflow-preview-chat" data-tenant={tenantKey} data-mentor={mentorId}>
      Preview Chat
    </div>
  ),
}));
...
vi.mock('@/components/workflows', () => ({
  ...
  WorkflowSidebar: ({
    ...
    <div data-testid="workflow-sidebar" data-nodetypes={JSON.stringify(nodeTypes)}>
      <button
      ...
      <button
        ...
        onMouseDown={() => onDragStart({ id: 'test', label: 'Test', type: 'test' })}
      >
        Drag Node
      </button>
    </div>
  ...
  DeleteWorkflowModal: ({
    ...
    isOpen ? (
      <div data-testid="delete-modal" data-name={workflowName}>
        <button data-testid="delete-confirm" onClick={onConfirm} disabled={isDeleting}>
...
describe('WorkflowDetailPage', () => {
  beforeEach(() => {
    ...
    mockActivateWorkflow.mockReturnValue({
      unwrap: vi.fn().mockResolvedValue({ is_valid: true, errors: [], warnings: [] }),
    });
  ...
  describe('back navigation', () => {
    it('should navigate back when clicking back button', async () => {
      ...
      const backButton = backButtons.find((btn) => btn.className.includes('text-muted-foreground'));
      if (backButton) {
  ...
  describe('publish workflow', () => {
    ...
    it('should show validation errors on publish failure with validation data', async () => {
      ...
      await waitFor(() => {
        expect(toast.error).toHaveBeenCalledWith('Workflow has validation issues');
      });
    });
  ...
  describe('delete workflow', () => {
    ...
    it('should delete workflow and navigate on confirm', async () => {
      ...
      await waitFor(() => {
        expect(mockPush).toHaveBeenCalledWith('/platform/test-tenant/workflows/original-mentor');
      });
    });
  ...
  describe('preview mode', () => {
    it('should enter preview mode when clicking Preview', async () => {
      ...
      expect(screen.getByTestId('workflow-canvas')).toHaveAttribute('data-preview', 'true');
    });
    ...
    it('should exit preview mode when clicking Close preview', async () => {
      ...
      expect(screen.queryByTestId('workflow-preview-chat')).not.toBeInTheDocument();
    });
  ...
  describe('node types transformation', () => {
    it('should transform node types for sidebar', () => {
      ...
      const nodeTypes = JSON.parse(sidebar.getAttribute('data-nodetypes') || 'null');
    ...
    it('should filter out start node from sidebar', () => {
      ...
      if (nodeTypes) {
        const allItems = nodeTypes.flatMap((section: { items: { id: string }[] }) => section.items);
        const startNode = allItems.find((item: { id: string }) => item.id === 'start');
    ...
    it('should override note category from visual to core', () => {
      ...
      if (nodeTypes) {
        ...
        const noteInCore = coreSection?.items.find((item: { id: string }) => item.id === 'note');

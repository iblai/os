...
export async function generateShareableLink({
  ...
  const shareableLinkButton = page.locator(SHAREABLE_LINK_BUTTON_SELECTOR);
  await expect(shareableLinkButton).toBeVisible({ timeout: 30_000 });
  await shareableLinkButton.click();

  const shareableLink = await page.evaluate(() => {

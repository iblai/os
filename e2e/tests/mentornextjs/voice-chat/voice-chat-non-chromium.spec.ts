...
test.describe('Voice Chat (Firefox & WebKit)', () => {
  test.setTimeout(300000);

  test.skip(
    ({ browserName }) => browserName === 'chromium',
    'This suite targets Firefox and WebKit only'
  );
  ...
  test('user can initiate voice call on a newly created mentor', async ({
    ...
    // Assert the voice chat modal opens (microphone won't connect but dialog should appear)
    ...
    // Close the voice chat modal via the end call button

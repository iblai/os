...
test.describe('Journey 25: Mentor MCP Tab', () => {
  ...
  test('admin goes to MCP tab and sees connector list or empty state', async ({
    ...
    await expect(editMentorPage.mcp.addConnectorButton).toBeVisible({ timeout: 10_000 });
  ...
  test('admin goes to MCP tab and adds a new MCP connector which appears in the list', async ({
    editMentorPage,
  }) => {
  ...
  test('admin goes to MCP tab and deletes an MCP connector', async ({
    editMentorPage,
  }) => {
    ...
    const initialCount = await editMentorPage.mcp.deleteButtons.count().catch(() => 0);
    await editMentorPage.mcp.deleteFirst();
    await waitForPageReady(editMentorPage.page);
    const finalCount = await editMentorPage.mcp.deleteButtons.count().catch(() => 0);

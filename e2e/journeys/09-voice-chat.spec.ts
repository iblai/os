...
test.describe('Journey 9: Voice Chat', () => {
  ...
  test('authenticated user goes to chat page and opens the voice call dialog with heading, mute and end-call buttons', async ({
    ...
    const visible = await voiceCallBtn.isVisible({ timeout: 10_000 }).catch(() => false);
    if (!visible) {
    ...
    await expect(dialog.getByRole('heading', { name: /voice chat/i })).toBeVisible();
    await expect(
  ...
  test('admin goes to mentor settings and hides the voice call button by toggling off Show Voice Call', async ({
    ...
    const visible = await showVoiceSwitch.isVisible({ timeout: 5_000 }).catch(() => false);
    if (!visible) {
  ...
  test('admin goes to chat page and completes a full voice call flow using mocked call credentials and STT', async ({
    ...
    const isAdmin = await checkAdminStatus(page);
    test.skip(!isAdmin, 'Full voice flow requires admin access to create a mentor');
    ...
    const muteButton = voiceDialog.getByRole('button', { name: /mute microphone/i });

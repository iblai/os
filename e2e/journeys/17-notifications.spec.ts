...
test.describe('Journey 17: Notifications', () => {
  test.beforeEach(async ({ page }) => {
    await navigateToMentorApp(page);
  });

  test('authenticated user goes to sidebar notifications button and lands on the notification page', async ({
    page,
    notificationsPage,
  }) => {
  ...
  test('authenticated user goes to navbar and sees notification dropdown from the bell', async ({
    page,
    navbarPage,
  }) => {
    const bell = navbarPage.notificationBell;
    const visible = await bell.isVisible({ timeout: 10_000 }).catch(() => false);
    if (!visible) return;
    await bell.click();
    const dropdown = page
      .getByRole('dialog')
      .or(page.locator('[class*="notification-dropdown"]'));
  ...
  test('admin goes to notifications page and sees the Mark All as Read button', async ({
    ...
    await notificationsPage.goto();
    await expect(notificationsPage.markAllReadButton).toBeVisible({ timeout: 10_000 });
  });
  ...
  test('admin goes to notifications alerts tab and sees proactive fields with ARIA attributes', async ({
    ...
    const tabRole = await notificationsPage.alertsTab.getAttribute('role').catch(() => null);
  ...
  test('admin goes to notifications page with empty inbox and the alerts tab auto-opens', async ({
    ...
    if (alertsActive) {
      await expect(notificationsPage.alertsContent).toBeVisible({ timeout: 10_000 });

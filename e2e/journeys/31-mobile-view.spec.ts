...
test.describe("Journey 31: Mobile View", () => {
  ...
  test("non-admin on mobile goes to sidebar and sees the correct menu items", async ({
    nonadminPage,
    nonadminSidebarPage,
  }) => {
  ...
  test("non-admin on mobile goes to explore page and sees the title, description, and tabs", async ({
    nonadminPage,
    nonadminSidebarPage,
    nonadminExplorePage,
  }) => {
  ...
  test("non-admin on mobile goes to explore page and searches for a mentor", async ({
    nonadminPage,
    nonadminSidebarPage,
    nonadminExplorePage,
  }) => {
  ...
  test("non-admin on mobile goes to datasets tab and performs upload and untrain/delete flow", async ({
    nonadminPage,
    ...
    await expect(nonadminEditMentorPage.datasets.addResourceButton).toBeVisible(
      { timeout: 10_000 },
    );

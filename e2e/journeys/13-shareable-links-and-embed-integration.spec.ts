...
test.describe('Journey 13: Shareable Links & Embed Integration', () => {
  ...
  test('admin goes to embed tab and configures a non-anonymous embed with voice call, voice record, and attachment buttons', async ({
    page,
    editMentorPage,
  }) => {
    await editMentorPage.open('Embed');
    await waitForPageReady(page);
    await expect(editMentorPage.embed.embedCodeBlock).toBeVisible({ timeout: 15_000 });
  ...
  test('admin goes to embed tab and an authenticated embed chat sends a message and receives a response', async ({
    ...
    await page.goto(EMBED_URL, { waitUntil: 'domcontentloaded', timeout: 30_000 });
  ...
  test('admin goes to embed tab and configures an advanced anonymous embed with Anyone visibility', async ({
    page,
    editMentorPage,
  }) => {

...
test.describe('Journey 22: Disclaimers & User Agreement', () => {
  ...
  test('admin goes to disclaimers tab, toggles User Agreement on, and an unauthenticated user sees the modal before chat', async ({
    ...
    await expect(editMentorPage.disclaimers.activeStatus).toBeVisible({ timeout: 5_000 });
    ...
    try {
      await anonPage.goto(mentorUrl, { waitUntil: 'domcontentloaded', timeout: 60_000 });
      await safeWaitForURL(anonPage, (url) => url.href.includes('/platform/'), { timeout: 30_000 });
      ...
      if (await chatInput.isVisible({ timeout: 15_000 }).catch(() => false)) {
        ...
        if (await sendBtn.isEnabled({ timeout: 5_000 }).catch(() => false)) {
          await sendBtn.click();
          const modal = anonPage.getByRole('dialog').filter({ hasText: 'User Agreement' });
  ...
  test('admin goes to disclaimers tab and edits the User Agreement content', async ({
    editMentorPage,
  }) => {
    await expect(editMentorPage.disclaimers.editAgreementButton).toBeVisible({ timeout: 10_000 });
    await editMentorPage.disclaimers.editAgreementButton.click();
  ...
  test('admin goes to disclaimers tab and copies the User Agreement content to clipboard', async ({
    ...
    const copyButton = editMentorPage.dialog.getByRole('button', { name: /copy/i });
    const visible = await copyButton.isVisible({ timeout: 5_000 }).catch(() => false);
    if (!visible) return;
  ...
  test('admin goes to disclaimers tab and edits the Advisory text which appears above the chat input', async ({
    editMentorPage,
  }) => {
    const advisoryField = editMentorPage.disclaimers.advisoryTextarea;
    const visible = await advisoryField.isVisible({ timeout: 5_000 }).catch(() => false);
    if (!visible) return;
    await advisoryField.fill('E2E advisory test text');
    const saveBtn = editMentorPage.disclaimers.saveButton;
  ...
  test('admin goes to disclaimers tab and the User Agreement modal is accessible', async ({
    ...
    await editMentorPage.disclaimers.enableUserAgreement();
    const modal = page.getByRole('dialog').filter({ hasText: 'User Agreement' });
  ...
  test('admin creates a new mentor, configures disclaimers, and an unauthenticated user sees the disclaimer modal before chat', async ({
    page,
    editMentorPage,
    browser,
  }) => {
    ...
    const visible = await newMentorBtn.isVisible({ timeout: 5_000 }).catch(() => false);
    if (!visible) return;
    await newMentorBtn.click();

    const createDialog = page.getByRole('dialog', { name: /create.*mentor|new mentor/i });
    await expect(createDialog).toBeVisible({ timeout: 10_000 });
    const nameInput = createDialog.getByPlaceholder(/mentor name|name/i).first();
    await nameInput.fill(`E2E Disclaimer Test Mentor ${Date.now()}`);
    const createBtn = createDialog.getByRole('button', { name: /create|save/i }).last();
    await createBtn.click();
    await safeWaitForURL(page, (url) => url.href.includes('/platform/'), { timeout: 30_000 });

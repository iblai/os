...
test.describe('Journey 11: Canvas — Embed', () => {
  ...
  test.fixme(
    'admin goes to a Canvas LMS embed and verifies it displays correctly and allows chatting',
    async ({ page }) => {
      test.skip(
        !hasCanvasEnv,
      ...
      const iframe = page.frameLocator(
        'iframe[src*="mentor"], iframe[title*="mentor"]',
      );
      const chatInput = iframe.getByPlaceholder('Ask anything', {
        exact: true,
      });

...
const PDF_FILE   = path.join(FILES_DIR, '0028-oop-object-oriented-programming-using-cpp.pdf');
...
test.describe('Journey 20: Dataset Management', () => {
  ...
  test('admin goes to datasets tab and verifies the header and description display correctly', async ({
    ...
    const header = editMentorPage.dialog.getByRole('heading', { name: /datasets/i });
  ...
  test('admin goes to datasets tab and verifies the search input is visible', async ({
    ...
    await expect(editMentorPage.datasets.searchInput).toBeVisible({ timeout: 10_000 });
  });
  ...
  test('admin goes to datasets tab and tests pagination controls when datasets exist', async ({
    ...
    if (visible && await nextBtn.isEnabled({ timeout: 3_000 }).catch(() => false)) {
      await nextBtn.click();
  ...
  test('admin goes to datasets tab and tests the training status switch when datasets exist', async ({
    ...
    const visible = await trainingSwitch.isVisible({ timeout: 5_000 }).catch(() => false);
    if (visible) {
  ...
  test('admin goes to datasets tab and tests the Schedule Retrain button when available', async ({
    ...
    const visible = await scheduleBtn.isVisible({ timeout: 5_000 }).catch(() => false);
    if (visible) {
      await scheduleBtn.click();
      const modal = editMentorPage.page.getByRole('dialog').filter({ hasText: /retrain/i });
  ...
  test('admin goes to datasets tab and uploads a PDF file successfully', async ({
    ...
    const chooserVisible = await fileInput.isVisible({ timeout: 3_000 }).catch(() => false);
    if (chooserVisible) {
  ...
  test('admin goes to datasets tab and tests Train or Delete modal actions for an untrained dataset', async ({
    ...
    if (switchCount === 0) { logger.info('TC13: No training switches found'); return; }
    ...
    for (let i = 0; i < switchCount; i++) {
      ...
      if ((await sw.getAttribute('aria-checked')) === 'false') {
        ...
        const modal = page.getByRole('dialog').filter({ hasText: /What would you like to do/i });
        ...
        if (modalVisible) {
          ...
          await expect(modal.getByText(/This dataset is currently untrained/i)).toBeVisible();
  ...
  test('admin goes to datasets tab and verifies dataset name links are clickable and have correct attributes', async ({
    ...
    await page.waitForTimeout(3_000);
    const links = editMentorPage.dialog.locator('a[href]').filter({ hasText: /.+/ });
  ...
  test('admin goes to datasets tab and closes the datasets dialog properly using the close button', async ({
    ...
    const dialogVisible = await editMentorPage.dialog.isVisible().catch(() => false);
    if (!dialogVisible) {
  ...
  test('admin goes to datasets tab and verifies the token count displays in the correct numeric format', async ({
    ...
    const rowCount  = await tableBody.locator('tr').count().catch(() => 0);
    if (rowCount > 0) {
  ...
  test('admin goes to datasets tab and verifies proper tooltips appear on the action buttons', async ({
    ...
    const hasScheduleTrigger = (await scheduleTooltipTriggers.count().catch(() => 0)) > 0;
    if (hasScheduleTrigger) {
      await scheduleTooltipTriggers.first().hover({ force: true });
      await page.waitForTimeout(500);
      const tooltip = page.locator('[role="tooltip"]');
      const tooltipVisible = await tooltip.isVisible().catch(() => false);
      logger.info(`TC19: Schedule retrain tooltip: ${tooltipVisible ? 'visible' : 'not visible'}`);
    } else {
      const scheduleBtns = editMentorPage.dialog.getByRole('button').filter({ has: page.locator('svg.lucide-clock') });
  ...
  test('admin goes to datasets tab and uploads multiple different file types successfully', async ({
    ...
    for (const resource of resources) {
      ...
      await addBtn.click();
      const addModal = page.getByRole('dialog').filter({ hasText: /Add Resources/i });
      await expect(addModal).toBeVisible({ timeout: 15_000 });
      const typeBtn = addModal.locator('button').filter({ hasText: new RegExp(`^${resource.type}$`, 'i') });
      await expect(typeBtn).toBeVisible({ timeout: 5_000 });
      await typeBtn.click();
      await page.waitForTimeout(1_000);
      const typeDialog = page.getByRole('dialog').filter({ hasText: resource.type });
      await expect(typeDialog).toBeVisible({ timeout: 10_000 });
      await typeDialog.locator('input[type="file"]').setInputFiles(resource.path);
      await page.waitForTimeout(3_000);
  ...
  test('admin goes to datasets tab and trains an untrained dataset by clicking Train in the modal', async ({
    ...
    await page.waitForTimeout(3_000);
    const switches = editMentorPage.dialog.getByRole('switch', { name: /training for document/i });
    const count = await switches.count().catch(() => 0);
    if (count === 0) { logger.info('TC24: No training switches found'); return; }
    for (let i = 0; i < count; i++) {
      ...
      if ((await sw.getAttribute('aria-checked')) === 'false') {
        ...
        if (await modal.isVisible({ timeout: 5_000 }).catch(() => false)) {
          ...
          logger.info(`TC24: Training initiated — In Progress badge: ${badgeVisible}`);
        }
        break;
      }
    }
  ...
  test('admin goes to datasets tab and deletes an untrained dataset by clicking Delete in the modal', async ({
    ...
    if (initial === 0) { logger.info('TC25: No datasets found'); return; }
  ...
  test('admin goes to datasets tab and untrains then deletes a trained dataset', async ({
    ...
    const switches = editMentorPage.dialog.getByRole('switch', { name: /training for document/i });
    const initial = await switches.count().catch(() => 0);
    if (initial === 0) { logger.info('TC26: No datasets found'); return; }
    for (let i = 0; i < initial; i++) {
      const sw = switches.nth(i);
      if ((await sw.getAttribute('aria-checked')) === 'true') {
        await sw.click();
        await page.waitForTimeout(1_000);
        const modal = page.getByRole('dialog').filter({ hasText: /Delete Dataset/i });
        if (await modal.isVisible({ timeout: 5_000 }).catch(() => false)) {
          await modal.getByRole('button', { name: /Delete/i }).click();
          await page.waitForTimeout(2_000);
          const after = await switches.count().catch(() => 0);
  ...
  test('admin goes to datasets tab and schedules retraining for a trained dataset', async ({
    ...
    if (count === 0) { logger.info('TC27: No schedule buttons found'); return; }
    for (let i = 0; i < count; i++) {
      const btn = scheduleBtns.nth(i);
      if (!await btn.isDisabled().catch(() => true)) {
        await btn.click();
        const modal = page.getByRole('dialog').filter({ hasText: /Schedule Retraining/i });
        if (await modal.isVisible({ timeout: 5_000 }).catch(() => false)) {
          ...
          await modal.getByRole('button', { name: /Schedule Retraining/i }).click();
          await page.waitForTimeout(2_000);
          logger.info('TC27: Dataset retraining scheduled');
        }
        break;
      }
    }
  ...
  test('admin goes to datasets tab and handles file upload cancellation gracefully', async ({
    ...
    const isDialogVisible = await editMentorPage.dialog.isVisible().catch(() => false);
    if (isDialogVisible) {
      await expect(addBtn).toBeVisible({ timeout: 5_000 });
      await expect(addBtn).toBeEnabled();
      logger.info('TC28: Dialog remained open and functional after cancellation');
    } else {
      logger.info('TC28: Dialog closed after cancellation — page still functional');
    }
  });
});

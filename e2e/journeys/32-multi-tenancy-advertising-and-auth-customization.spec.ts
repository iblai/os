...
import { navigateToMentorApp, checkAdminStatus, authenticate } from '../utils/auth';
...
test.describe('Journey 32: Multi-Tenancy, Advertising & Auth Customization', () => {
  ...
  test('admin goes to enterprise tenant and creates a new mentor from the sidebar dialog', async ({
    ...
    if (!await newMentorBtn.isVisible({ timeout: 5_000 }).catch(() => false)) return;
    await newMentorBtn.click();
    const dialog = page.getByRole('dialog', { name: /create.*mentor|new mentor/i });
  ...
  test('admin goes to enterprise tenant and creates a new mentor from the Settings dialog', async ({
    ...
    if (!await settingsBtn.isVisible({ timeout: 5_000 }).catch(() => false)) return;
  ...
  test('admin goes to enterprise tenant and creates a new mentor from the My Mentors dialog', async ({
    ...
    const dialog = page.getByRole('dialog');
    await expect(dialog).toBeVisible({ timeout: 10_000 });
    const createBtn = dialog.getByRole('button', { name: /create|new mentor/i });
    const visible = await createBtn.isVisible({ timeout: 5_000 }).catch(() => false);
    if (visible) await expect(createBtn).toBeVisible();
    await page.keyboard.press('Escape');
  });
  ...
  test('authenticated user goes to enterprise tenant and the platform logo navigates home', async ({
    ...
    const logo = page.getByRole('link', { name: /home|logo/i })
      .or(page.locator('[data-testid="platform-logo"]'))
      .first();
    if (await logo.isVisible({ timeout: 5_000 }).catch(() => false)) {
      await logo.click();
      await safeWaitForURL(page, (url) => url.href.includes('/platform/'), { timeout: 15_000 });
  ...
  test('unauthenticated user goes to advertising tenant mentor page and can access it without logging in', async ({
    ...
    test.skip(!FORDHAM_HOST, 'Set FORDHAM_HOST to enable advertising tenant test');
    const anonContext = await browser.newContext({ storageState: undefined });
    const anonPage = await anonContext.newPage();
    try {
      await anonPage.goto(FORDHAM_HOST, { waitUntil: 'domcontentloaded', timeout: 60_000 });
      ...
      const hasLoginOrChat = await loginButton
  ...
  test('admin goes to auth SPA customization settings and an unauthenticated user sees the customization in the auth SPA', async ({
    ...
    test.skip(!AUTH_NEXTJS_HOST, 'Set AUTH_NEXTJS_HOST to enable auth customization test');
    const isAdmin = await checkAdminStatus(page);
    test.skip(!isAdmin, 'Requires admin access');
    ...
    const anonPage = await anonContext.newPage();
    try {
      await anonPage.goto(AUTH_NEXTJS_HOST, { waitUntil: 'domcontentloaded', timeout: 60_000 });
      await waitForPageReady(anonPage);
  ...
  test('admin goes to help center settings and toggles its visibility in dropdown and embed', async ({
    ...
    const helpCenterToggle = editMentorPage.dialog.getByRole('switch', { name: /help center/i });
    if (await helpCenterToggle.isVisible({ timeout: 5_000 }).catch(() => false)) {
      const wasEnabled = (await helpCenterToggle.getAttribute('aria-checked')) === 'true';
      await helpCenterToggle.click();
      await page.waitForTimeout(500);
      if (wasEnabled !== ((await helpCenterToggle.getAttribute('aria-checked')) === 'true')) {
  ...
  test('admin goes to help center settings and updates the help center URL in dropdown and embed menu', async ({
    ...
    await editMentorPage.open('Settings');
    await waitForPageReady(page);
    const helpUrlInput = editMentorPage.dialog.getByLabel(/help.*url|help center url/i)
    ...
    if (await helpUrlInput.isVisible({ timeout: 5_000 }).catch(() => false)) {
      ...
      const saveBtn = editMentorPage.dialog.getByRole('button', { name: /save/i }).first();
  ...
  test('unauthenticated user goes to advertising tenant mentor page and logs in', async ({
    ...
    test.skip(!FORDHAM_HOST, 'Set FORDHAM_HOST to enable advertising tenant login test');
    ...
    try {
      ...
      await safeWaitForURL(anonPage, (url) => url.href.includes('login'), { timeout: 60_000 });

...
> Last updated: 2026-08-07 | 620 checkpoints (590 covered, 8 pending/fixme, 10 not-reproducible in default env, 12 deprecated) | 68 journeys (67 active, 1 deprecated in #1431) | 100% covered | Auth: admin + non-admin storageState
...
- [x] Clicking a pagination page number pushes `datasetsPage` into the URL via `router.push` (TC36, seeded multi-page mentor)
- [x] Browser Back/Forward walk the visited `datasetsPage` values after paging forward twice (TC37, seeded multi-page mentor)
- [x] Searching while on page 3 drops `datasetsPage` (reset to page 1) and sets `datasetsSearch` (TC38, seeded multi-page mentor)
- [x] Reloading the page while paginated restores `datasetsPage` and the active pagination link from the URL (TC39, seeded multi-page mentor)
...
The backend does not expose the grader RBAC permissions yet (`graderconfigurations` / `gradercriteria`), so the published SDK build ships the grader tab without RBAC wiring (its RBAC props were removed until the permissions land) and the host gates the segment to platform admins via `userTypes: [ADMIN]` instead (mirroring Tasks / LTI). When the permissions land, the SDK re-adds its in-tab checks and the host gates the segment on `graderconfigurations/#read` (hiding the tab outright for a denied admin), with denied `write`/`action`/`delete` omitting the Save/Add/Edit/Delete affordances rather than erroring — see grd-13.
...
- [ ] grd-13 _(not-reproducible)_: the grader tab's fine-grained RBAC gating (denied `graderconfigurations/#read` renders the denied state; denied `write`/`action`/`delete` on config or criteria resources omits the Save/Add/Edit/Delete affordances) — currently doubly unreachable: the backend does not expose the grader permissions yet and the SDK build ships the tab without RBAC wiring, with the host gating the segment to admins via `userTypes` instead; once the permissions land it will still need a fixture seeding a restricted RBAC permission object for the e2e admin, which this environment has no fixture for

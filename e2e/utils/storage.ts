...
export async function seedRedirectParams(
  ...
  await page.addInitScript((p) => {
    ...
    if (p.selectedTenant) localStorage.setItem('selected_tenant', p.selectedTenant);
...
export async function seedAuthTokens(
  ...
  await page.addInitScript((t) => {
    ...
    if (t.dmTokenExpires) localStorage.setItem('dm_token_expires', t.dmTokenExpires);
    if (t.axdToken) localStorage.setItem('axd_token', t.axdToken);
    if (t.axdTokenExpires) localStorage.setItem('axd_token_expires', t.axdTokenExpires);

...
# 4. Permissions the WebView needs at runtime. wry's chrome client asks for
# CAMERA / RECORD_AUDIO / MODIFY_AUDIO_SETTINGS when a page calls getUserMedia,
# but Android silently denies anything the manifest does not declare — so
# without these the QR scanner, camera capture, voice chat and calls all fail.
# gen/android is regenerated by `cargo tauri android init` with INTERNET only,
# hence the patch lives here with the rest of the branding (idempotent).
MANIFEST="src-tauri/gen/android/app/src/main/AndroidManifest.xml"
python3 - "$MANIFEST" <<'PY'
import sys, re
p = sys.argv[1]
s = open(p).read()  # raises if gen/android is missing: run `cargo tauri android init` first
perms = [
...
features = [
    # Both optional: phones without them must still be able to install the app.
    '<uses-feature android:name="android.hardware.camera" android:required="false" />',
    '<uses-feature android:name="android.hardware.microphone" android:required="false" />',
]
added = []
anchor = '<uses-permission android:name="android.permission.INTERNET" />'
assert anchor in s, "INTERNET permission anchor not found"
for perm in perms:
...
for feat in features:
    name = re.search(r'android:name="([^"]+)"', feat).group(1)
    if f'android:name="{name}"' not in s:
        s = s.replace(anchor, anchor + "\n    " + feat, 1)
        added.append(name.split(".")[-1])
open(p, "w").write(s)
print("manifest permissions:", ", ".join(added) if added else "already present")
PY

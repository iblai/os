...
export const messagesApiSlice = createApi({
  ...
  endpoints: (builder) => ({
    getRecentMessages: builder.query<GetRecentMessagesResponse, GetRecentMessagesArgs>({
      query: (args) => ({
        url: MESSAGES_ENDPOINTS.GET_RECENT_MESSAGES.path(args.tenantKey, args.username),
...
export const { useGetRecentMessagesQuery, useLazyGetRecentMessagesQuery } = messagesApiSlice;
